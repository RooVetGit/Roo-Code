...
const mockUPlot = jest.fn().mockImplementation((options, data) => {
...
describe("<CostTrendChart />", () => {
	...
	beforeEach(() => {
		...
		lastUPlotData = null
		if (mockUPlotInstance) mockUPlotInstance.cursor.idx = null
	})
	...
	it('should display "No API requests made yet" message when chartData is empty or invalid', () => {
		renderChart({ chartData: undefined })
		expect(screen.getByText(/No API requests made yet/i)).toBeInTheDocument()

		renderChart({ chartData: [[], []] })
		expect(screen.getByText(/No API requests made yet/i)).toBeInTheDocument()

		renderChart({ chartData: [undefined, undefined] })
		expect(screen.getByText(/No API requests made yet/i)).toBeInTheDocument()
	})
	...
	it("should attempt to read CSS variables on mount", () => {
		...
		expect(mockGetComputedStyle().getPropertyValue).toHaveBeenCalledWith("--vscode-foreground")
		expect(mockGetComputedStyle().getPropertyValue).toHaveBeenCalledWith("--vscode-button-foreground")
		expect(mockGetComputedStyle().getPropertyValue).toHaveBeenCalledWith("--vscode-editor-background")
	})
	...
	it('should configure uPlot sync with GRID_SYNC key when syncKey is "grid"', () => {
		...
		expect(mockUPlot).toHaveBeenCalled()
		const mockedSync = require("uplot").sync
		expect(mockedSync).toHaveBeenCalledWith("cost-charts-grid")
