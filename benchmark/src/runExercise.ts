...
export async function run() {
	...
	const usage = await waitUntilCompleted({ api, taskId, timeout: 10 * 60 * 1_000 }) // 10m

	if (usage) {
		const result = { runId: parseInt(runId), ...usage, duration: Date.now() - startTime }
		console.log(result)
		await fs.writeFile(path.resolve(workspacePath, "usage.json"), JSON.stringify(result, null, 2))
	}
}
