...
import { bedrockDefaultModelId, bedrockModels, bedrockDefaultPromptRouterModelId } from "../../shared"
...
export class AwsBedrockHandler extends BaseProvider {
	...
	async *createMessage(systemPrompt, messages) {
		...
		const inferenceConfig = {
			maxTokens: modelConfig.info.maxTokens,
	...
	static prefixIsMultiRegion(arnPrefix) {
		for (const [prefix, info] of Object.entries(AMAZON_BEDROCK_REGION_INFO)) {
			if (arnPrefix === prefix) {
...
2. Or, create an inference profile and then leverage the "Use custom ARN..." option of the model selector in settings.`,
	 ...
	 */
	formatErrorMessage(error, errorType, isStreamContext) {
