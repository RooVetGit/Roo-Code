...
describe("AwsBedrockHandler", () => {
	...
	describe("getModel", () => {
		...
		it("should handle inference-profile ARN with apne3 region prefix", () => {
			...
			const parseArnMock = jest.fn().mockImplementation(function (arn, region) {
			...
			try {
				...
				expect(customArnHandler.arnInfo.modelId).toBe("anthropic.claude-3-5-sonnet-20241022-v2:0")
				// Verify cross-region inference flag is false since apne3 is a prefix for a single region
				expect(customArnHandler.arnInfo.crossRegionInference).toBe(false)
			} finally {
