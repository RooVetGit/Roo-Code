...
describe("OpenAiNativeHandler", () => {
	...
	describe("streaming models", () => {
		...
		it("should handle streaming response", async () => {
			...
			// Use type assertion to avoid TypeScript errors
			expect(results[3].inputTokens).toBe(10)
			expect(results[3].outputTokens).toBe(5)
			expect(results[3].totalCost).toBeCloseTo(0.00006, 6)
			expect(mockCreate).toHaveBeenCalledWith({
				model: "gpt-4.1",
				temperature: 0,
				messages: [
		...
		it("should handle empty delta content", async () => {
			...
			// Use type assertion to avoid TypeScript errors
			expect(results[1].inputTokens).toBe(10)
			expect(results[1].outputTokens).toBe(5)
			expect(results[1].totalCost).toBeCloseTo(0.00006, 6)
		})
	})
