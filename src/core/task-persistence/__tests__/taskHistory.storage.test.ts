...
import { setHistoryItems, getHistoryItem, deleteHistoryItem, reconstructTask } from "../taskHistory"
...
describe("taskHistory.ts - Core Storage Operations", () => {
	...
	describe("taskHistory.ts - Advanced setHistoryItems Tests", () => {
		...
		describe("taskHistory.ts - getHistoryItem() Advanced Tests", () => {
			...
			test("should trigger file read on cache miss", async () => {
				...
				expect(vi.mocked(safeReadJson)).toHaveBeenCalledWith(
					expect.stringContaining("cache-miss-test/history_item.json"),
				)
			})
			...
			test("should bypass cache when useCache=false", async () => {
				...
				vi.mocked(safeReadJson).mockResolvedValue(updatedItem)
				...
				expect(vi.mocked(safeReadJson)).toHaveBeenCalled()
				expect(vi.mocked(safeReadJson)).toHaveBeenCalledWith(
					expect.stringContaining("task-123/history_item.json"),
				)
			})
			...
			test("should suppress ENOENT errors", async () => {
				...
				// Verify console.error was not called for ENOENT
			...
			test("should handle other file system errors", async () => {
				...
				// Verify console.error was called for non-ENOENT errors
				expect(consoleErrorSpy).toHaveBeenCalled()

				// Check that the first argument contains the expected substrings
				const firstCallFirstArg = consoleErrorSpy.mock.calls[0][0]
				expect(firstCallFirstArg).toContain("[TaskHistory] [getHistoryItem]")
				expect(firstCallFirstArg).toContain("permission-error")
				expect(firstCallFirstArg).toContain("error reading file")

				// Check that the second argument is the error object
				const firstCallSecondArg = consoleErrorSpy.mock.calls[0][1]
				expect(firstCallSecondArg).toBeInstanceOf(Error)
				expect(firstCallSecondArg.code).toBe("EACCES")
			})
		})
	...
	describe("reconstructTask() Tests", () => {
		// Mock data
		const mockTaskId = "task-reconstruct-123"
		...
		test("should successfully reconstruct task from UI messages", async () => {
			// Setup mocks
			vi.mocked(safeReadJson).mockImplementation(async (path) => {
		...
		test("should handle JSON parsing errors in api_req_started messages", async () => {
			...
			expect(consoleWarnSpy).toHaveBeenCalled() // Should log a warning
		})
		...
		test("should handle unclosed/malformed JSON in UI messages file", async () => {
			...
			expect(consoleErrorSpy).toHaveBeenCalled() // Should log an error
		...
		test("should handle directory size calculation errors", async () => {
			...
			expect(consoleWarnSpy).toHaveBeenCalled() // Should log a warning
			expect(consoleWarnSpy.mock.calls[0][0]).toContain("[Reconstruct Task] Could not calculate size")
		})
