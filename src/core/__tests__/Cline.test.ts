...
describe("Cline", () => {
	...
	describe("getEnvironmentDetails", () => {
		...
		beforeEach(() => {
			...
			Date.prototype.getTimezoneOffset = jest.fn(() => 420) // UTC-7 (420 minutes)
		})
		...
		it("should include timezone information in environment details", async () => {
			...
			// Call the function without injecting date; it should use mocked Date.now and TZ env var
			const details = await cline["getEnvironmentDetails"](false)

			// Verify the new time formats are present
			expect(details).toContain("# Current Zulu Date/Time\n2024-01-01T12:00:00.000Z")
			// Use regex for local time string due to potential variations in timezone name formatting
			expect(details).toMatch(/# Current Local Date\/Time\nMon Jan 01 2024 13:00:00 GMT[+-]\d{4} \(.*\)/)
		})
	}) // Close the describe("getEnvironmentDetails", ...) block here
	...
	describe("API conversation handling", () => {
		...
		it("should handle image blocks based on model capabilities", async () => {
			...
			jest.spyOn(clineWithoutImages as any, "loadContext").mockImplementation(async (content) => [content, ""])
