...
export function getCapabilitiesSection(
	...
	const cap_supportsComputerUse_0 = `${
		supportsComputerUse ? ", use the browser" : ""
	}`
	const diffStrategy_0 = `${diffStrategy ? "the apply_diff or write_to_file" : "the write_to_file"}`
	return `{capabilities.ts}
<RooJson>{"\${cwd}":"${cwd}","\${capabilities.ts-0}":"${diffStrategy_0}", "\${capabilities.ts-1}":"${cap_supportsComputerUse_0}", "\${capabilities.ts-2}":"${endstr}"}`
	return `====
...
- You can use the execute_command tool to run commands on the user's computer whenever you feel it can help accomplish the user's task. When you need to execute a CLI command, you must provide a clear explanation of what the command does. Prefer to execute complex CLI commands over creating executable scripts, since they are more flexible and easier to run. Interactive and long-running commands are allowed, since the commands are run in the user's VSCode terminal. The user may keep commands running in the background and you will be kept updated on their status along the way. Each command you execute is run in a new terminal instance.${endstr}`
}
