...
export function generateReadFileSchema(args: ToolArgs): BaseToolSchema {
	...
	const schema: BaseToolSchema = {
		name: "read_file",
		description: `Reads the content of one or more files, optionally within specific line ranges. Can read up to ${maxConcurrentReads} files at once. Supports text extraction from PDF and DOCX formats.`,
		parameters: [
			{
				name: "args",
				type: "object",
				description: "Container for file reading arguments.",
				required: true,
				properties: {
					file: {
						name: "file",
						type: "array",
						description: `An array of file objects to read, with an optional line range. Up to ${maxConcurrentReads} files can be read in a single call.`,
						required: true,
						items: {
							name: "fileItem",
							type: "object",
							description: "A file object specifying the path and optional line range.",
							required: true,
							properties: {
								path: {
									name: "path",
									type: "string",
									description: `The relative path to the file to be read. The path is relative to the workspace directory: ${args.cwd}.`,
									required: true,
								},
								...(partialReadsEnabled
									? {
											line_range: {
												name: "line_range",
												type: "string",
												description: `An optional line range to read from the file, formatted as 'start-end' (1-based, inclusive). Use this to read specific portions of large files efficiently.`,
												required: false,
											},
										}
									: {}),
