...
async function codebase(conversation: string, mcpHub: McpHub, codeContextServer: McpServer, cline: Task, n_results: number = 64) {
...
- Avoid repetition. Select the most critical keywords or dependencies, with no more than 4 items in total. Keywords with similar meanings or keywords used for the same purpose should not exceed 2.
- ** Each keywords should be written as a single line, starting with "-", and should not exceed 20 characters. **
- ** Do not output any information except the keywords **.
 ...
 * @returns 生成的MCP提示内容
 */
export async function generateCodebasePrompt(mcpHub: McpHub | undefined, conversation: Array<any>, cline: Task): Promise<string> {
  ...
  try {
    ...
    const {
      queries,
      contextContent
    } = await codebase(searchQuery, mcpHub, codeContextServer, cline, 32)
...
`
      const dst = await codebase(searchQuery, mcpHub, codeContextServer, cline, 8)
...
export async function addCodebaseInToConversation(cleanConversationHistory: {
  ...
  cline: Task) {
    ...
    // const { mcpEnabled } = (await cline.providerRef.deref()?.getState()) ?? {}
...
export async function addSummaryInToConversation(cleanConversationHistory: {
  ...
  cline: Task, paths?: string[] | undefined) {
