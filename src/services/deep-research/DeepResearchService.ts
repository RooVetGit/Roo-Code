...
export class DeepResearchService {
	...
	private chatSystemPrompt() {
		...
		// return trimPrompt(`
		// 	You are a general answering assistant that can comply with any request. Today is ${now}.
	...
	private async handleDone(message: { role: "user" | "assistant"; content: string }) {
		this.messages.push(message)
		const content = await this.withLoading(this.generateChatCompletion())

		if (content) {
			await this.postMessage({ type: "research.output", text: JSON.stringify({ content }) })
		}
	}
	...
	private updateTokenUsage(usage: OpenAI.CompletionUsage | null | undefined) {
		if (!usage) {
			return
		}
