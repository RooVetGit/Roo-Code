...
import { fileExistsAtPath } from "../../utils/fs" // Assuming this utility exists and works as expected
...
export abstract class ShadowCheckpointService extends EventEmitter {
	...
	public async initShadowGit(onInit?: () => Promise<void>) {
		...
		if (await fileExistsAtPath(this.dotGitDir)) {
			...
			await this.writeExcludeFile()
			await this.stageAll(git) // stageAll will be modified
	...
	private async stageAll(git: SimpleGit) {
		await this.renameNestedGitRepos(true) // renameNestedGitRepos will also be modified
		...
		try {
			...
			throw error // Rethrow to ensure operation is marked as failed
		} finally {
			await this.renameNestedGitRepos(false)
		}
	}
	// --- CHANGE END: Modify stageAll to be more performant ---
	...
	// nested repos.
	// --- CHANGE START: Modify renameNestedGitRepos to use cached paths ---
	...
	public async saveCheckpoint(message: string): Promise<CheckpointResult | undefined> {
		try {
			...
			const startTime = Date.now()
			await this.stageAll(this.git) // This now uses the optimized stageAll
	...
	// --- CHANGE START: Modify getDiff for performance and accuracy ---
	public async getDiff({ from, to }: { from?: string; to?: string }): Promise<CheckpointDiff[]> {
		...
		if (!to) {
			await this.stageAll(this.git) // Uses the optimized stageAll
		}
	...
	public static async deleteBranch(git: SimpleGit, branchName: string) {
		...
		if (currentBranch === branchName) {
			...
			try {
				...
				try {
					console.log(
						`[ShadowCheckpointService#deleteBranch] Running gc --prune=now after deleting branch ${branchName}`,
					)
					// Using raw for --prune=now as simple-git's gc() doesn't directly support it.
					git.raw(["gc", "--prune=now", "--quiet"])
					console.log(
						`[ShadowCheckpointService#deleteBranch] gc --prune=now completed for branch ${branchName}`,
					)
				} catch (gcError) {
					console.error(
						`[ShadowCheckpointService#deleteBranch] gc after deleting branch ${branchName} failed: ${gcError instanceof Error ? gcError.message : String(gcError)}`,
					)
				}
			...
			try {
				...
				git.raw(["gc", "--prune=now", "--quiet"])
				console.log(`[ShadowCheckpointService#deleteBranch] gc --prune=now completed for branch ${branchName}`)
			} catch (gcError) {
				console.error(
					`[ShadowCheckpointService#deleteBranch] gc after deleting branch ${branchName} failed: ${gcError instanceof Error ? gcError.message : String(gcError)}`,
				)
